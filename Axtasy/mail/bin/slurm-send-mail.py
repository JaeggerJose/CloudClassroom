...
logging.basicConfig(level=logging.DEBUG, format='%(asctime)s %(levelname)-8s %(message)s', datefmt='%a, %d %b %Y %H:%M:%S', filename='/tmp/slurm_logger.txt', filemode='w')
...
def send_mail(user_email: str, first_job_id: int, state: str):
    
    jobs = []  # store job object for each job in this array
    logging.debug(f"INPUT: {user_email}, {first_job_id}, {state}")
    ...
    if state in ['Began', 'Ended', 'Failed']:
        ...
        # Sleep 3 secs for sacct to obtain the Start time
        time.sleep(3)
...
if __name__ == "__main__":
    ...
    #     )
    # sys.argv = ["/opt/slurm-mail/bin/slurm-send-mail.py", "-s", "Slurm Job_id=245 Name=slurm_email_test_job Began, Queued time 00:00:01", "{user's email}"]
    # sys.argv = ["/opt/slurm-mail/bin/slurm-send-mail.py", "-s", "Slurm Job_id=245 Name=slurm_email_test_job Ended, Run time 00:00:31, COMPLETED, ExitCode 0", "{user's email}"]
    # sys.argv = ["/opt/slurm-mail/bin/slurm-send-mail.py", "-s", "Slurm Job_id=243 Name=slurm_email_test_job Failed, Run time 00:00:00, FAILED, ExitCode 1", "{user's email}"]
    content = sys.argv[2]
    user_email = sys.argv[3]
    job_id = re.search(r"Job_id=(?P<job_id>[0-9]+)", content).group("job_id")
    action = re.search(r"Name=.*?\s(?P<action>[\w]+?),", content).group("action")
    try:
        send_mail(user_email, int(job_id), action)
    except Exception as e:
        logging.error(f"SEND MAIL ERROR: {traceback.format_exc()}")
